...
export async function listFiles(prefix = '') {
    try {
        ...
        const response = await googleBucket.getFiles(options);
...
export async function listAllFiles(prefix = '', maxFiles) {
    try {
        const allFiles = [];
        const allPrefixes = [];
        let nextPageToken;
...
export async function listFilesByExtension(prefix = '', extensions) {
    try {
        // Input validation
        ...
        const normalizedExtensions = extensions.map(ext => ext.toLowerCase().replace(/^\./, '') // Remove leading dot if present
        );
